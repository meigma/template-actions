...
Report non-security bugs through GitHub issues. Include the following details
when possible:
...
If you are reporting a security issue, stop and follow
...
4. Use Conventional Commit subjects, such as `feat: add config loader` or
   `fix: handle empty input`.
5. Rebuild the bundle (`moon run root:package`) and commit `dist/` when `src/`
   changes.
6. Make sure `moon run root:check` passes before requesting review.
...
moon run root:lint
moon run root:test
moon run root:package
```

## Release Changes

Release Please reads Conventional Commit subjects to build changelogs and
release PRs. Keep release-impacting commits clear; routine docs, CI, and
maintenance commits should use the appropriate non-release type.
